...
function NotMnist:__init(config)
    ...
    = xlua.unpack({ config },
        ...
        },
        {
            arg = 'train_dir',
            type = 'string',
            default = 'notMNIST_large',
            help = 'name of train_dir'
        },
        {
            arg = 'test_dir',
            type = 'string',
            default = 'notMNIST_small',
...
function NotMnist:loadTrainValid()
    local bad_png = {
        --these wont load as PNGs, ignore them
        ['A'] = {
    ...
    local inputs, targets = self:loadData(self._train_dir, self._download_url, bad_png)
    -- train
    local start = 1
    local size = math.floor(inputs:size(1) * (1 - self._valid_ratio))
    self:trainSet(self:createDataSet(inputs:narrow(1, start, size),
        targets:narrow(1, start, size),
        'train'))
    -- valid
    start = size + 1
    size = inputs:size(1) - start
    self:validSet(self:createDataSet(inputs:narrow(1, start, size),
        targets:narrow(1, start, size),
        'valid'))
...
function NotMnist:loadTest()
    ...
    local inputs, targets = self:loadData(self._test_dir, self._download_url, bad_png)
...
function NotMnist:loadData(file_name, download_url, bad_png)
    local doc_path = DataSource.getDataPath {
        name = self._name,
        url = download_url .. file_name .. '.tar.gz',
