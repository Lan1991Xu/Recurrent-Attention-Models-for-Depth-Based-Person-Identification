...
function Svhn:__init(config)
    ...
    = xlua.unpack({ config },
        ...
        },
        {
            arg = 'train_file',
            type = 'string',
            default = 'housenumbers/train_32x32.t7',
            help = 'name of train_file'
        },
        {
            arg = 'extra_file',
            type = 'string',
            default = 'housenumbers/extra_32x32.t7',
            help = 'name of extra (yet easier) patterns used for training'
        },
        {
            arg = 'test_file',
            type = 'string',
            default = 'housenumbers/test_32x32.t7',
...
function Svhn:loadTrainValid()
    ...
    self:trainSet(self:createDataSet(self._input_tensor:narrow(1, 1, nSample - nValid),
        self._target_tensor:narrow(1, 1, nSample - nValid),
        'train'))

    -- valid
    self:validSet(self:createDataSet(self._input_tensor:narrow(1, nSample - nValid + 1, nValid),
...
function Svhn:loadTest()
    local doc_path = DataSource.getDataPath {
        name = self._name,
        url = self._download_url,
        decompress_file = self._test_file,
        data_dir = self._data_path
    }
    local data = torch.load(doc_path, 'ascii')
