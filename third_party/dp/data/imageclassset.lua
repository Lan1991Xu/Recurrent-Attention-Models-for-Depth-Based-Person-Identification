...
function ImageClassSet:__init(config)
    ...
    verbose, sort_func, cache_mode, cache_path = xlua.unpack({ config },
        'ImageClassSet',
...
function ImageClassSet:multithread(nThread)
    ...
    self._threads = threads.Threads(nThread,
        function()
            require 'dp'
        end,
        function(idx)
...
function ImageClassSet:subAsyncPut(batch, start, stop, callback)
    ...
    self._threads:addjob(-- the job callback (runs in data-worker thread)
        function()
