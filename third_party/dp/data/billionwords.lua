...
function BillionWords:__init(config)
    ...
    = xlua.unpack({ config },
        ...
        },
        {
            arg = 'train_file',
            type = 'string',
            default = 'train_data.th7',
            help = 'name of training file'
        },
        {
            arg = 'valid_file',
            type = 'string',
            default = 'valid_data.th7',
            help = 'name of validation file'
        },
        {
            arg = 'test_file',
            type = 'string',
            default = 'test_data.th7',
            help = 'name of test file'
        },
        {
            arg = 'word_file',
            type = 'string',
            default = 'word_map.th7',
...
function BillionWords:createSentenceSet(data, which_set)
    ...
    return dp.SentenceSet {
        ...
        end_id = self._sentence_end,
        start_id = self._sentence_start,
        words = self._classes,
...
function BillionWords:loadData(file_name, download_url)
    if not self._classes then
        local path = parent.getDataPath {
            name = self._name,
            url = download_url,
            decompress_file = self._word_file,
