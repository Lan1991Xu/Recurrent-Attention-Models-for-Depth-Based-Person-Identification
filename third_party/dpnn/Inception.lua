...
function Inception:__init(config)
    ...
    for i = 1, #self.kernelSize do
        ...
        -- 1x1 conv
        local reduce = nn.SpatialConvolution(self.inputSize, self.reduceSize[i], 1, 1,
        ...
        local conv = nn.SpatialConvolution(self.reduceSize[i], self.outputSize[i],
        ...
        local oWidth_i = torch.floor((iWidth + 2 * pad - self.kernelSize[i]) / self.kernelStride[i] + 1)
        local oHeight_i = torch.floor((iHeight + 2 * pad - self.kernelSize[i]) / self.kernelStride[i] + 1)
        if oWidth == nil then
            oWidth = oWidth_i
            oHeight = oHeight_i
        else
    ...
    if self.pool.kW ~= nil and self.pool.dW ~= nil and self.pool.padW ~= nil then
        assert(oWidth ~= nil)
        assert(oHeight ~= nil)
        local oWidth_pool = torch.floor((iWidth + 2 * self.pool.padW - self.pool.kW) / self.pool.dW + 1)
        local oHeight_pool = torch.floor((iHeight + 2 * self.pool.padH - self.pool.kH) / self.pool.dH + 1)
