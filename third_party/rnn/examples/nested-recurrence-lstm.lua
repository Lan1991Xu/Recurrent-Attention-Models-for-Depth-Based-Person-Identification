...
local rm = nn.Sequential():add(nn.ParallelTable():add(nn.LookupTable(nIndex, hiddenSize)):add(nn.Linear(hiddenSize, hiddenSize))):add(nn.CAddTable()):add(nn.Sigmoid()):add(nn.FastLSTM(hiddenSize, hiddenSize)) -- an AbstractRecurrent instance:add(nn.Linear(hiddenSize, hiddenSize)):add(nn.Sigmoid())

local rnn = nn.Sequential():add(nn.Recurrence(rm, hiddenSize, 0)) -- another AbstractRecurrent instance:add(nn.Linear(hiddenSize, nIndex)):add(nn.LogSoftMax())
