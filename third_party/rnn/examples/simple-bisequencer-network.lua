...
local fwd = nn.Recurrent(hiddenSize, nn.LookupTable(nIndex, hiddenSize),
...
local rnn = nn.Sequential():add(brnn):add(nn.Sequencer(nn.Linear(hiddenSize * 2, nIndex))) -- times two due to JoinTable:add(nn.Sequencer(nn.LogSoftMax()))
