...
local rm = nn.Sequential() -- input is {x[t], h[t-1]}:add(nn.ParallelTable():add(nn.LookupTable(nIndex, hiddenSize)) -- input layer:add(nn.Linear(hiddenSize, hiddenSize))) -- recurrent layer:add(nn.CAddTable()) -- merge:add(nn.Sigmoid()) -- transfer

local rnn = nn.Sequential():add(nn.Recurrence(rm, hiddenSize, 0)) -- similar to nn.Recurrent, but more general, and no startModule:add(nn.Linear(hiddenSize, nIndex)):add(nn.LogSoftMax())
