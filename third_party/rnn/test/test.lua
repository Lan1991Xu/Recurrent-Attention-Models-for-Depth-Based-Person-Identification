...
function rnntest.Recurrent_old()
    ...
    local rnn = nn.Recurrent(hiddenSize,
        nn.Linear(hiddenSize, hiddenSize),
        nn.Linear(hiddenSize, hiddenSize),
        nn.ReLU(), 99999)

    local rnn2 = nn.ReTest(rnn.startModule:clone(),
...
function rnntest.Recurrent_oneElement()
    ...
    -- RNN
    local r = nn.Recurrent(hiddenSize, nn.Linear(200, hiddenSize),
...
function rnntest.Recurrent_TestTable()
    ...
    feedbackModule:add(nn.Linear(hiddenSize, outputSize))
    -- rho = nStep
    local mlp = nn.Recurrent(nn.ParallelTable():add(nn.Add(outputSize)):add(nn.Add(outputSize)),
        nn.ParallelTable():add(inputModule:clone()):add(inputModule:clone()),
        nn.ParallelTable():add(feedbackModule:clone()):add(feedbackModule:clone()),
        nn.ParallelTable():add(transferModule:clone()):add(transferModule:clone()),
        nStep,
        nn.ParallelTable():add(nn.CAddTable()):add(nn.CAddTable()))
...
function rnntest.Sequencer_tensoreval()
    ...
    local lstm2 = lstm:clone()
    local seq = nn.Sequencer(lstm)
    local seq2 = nn.Sequential():add(nn.SplitTable(1)):add(nn.Sequencer(lstm2))
    seq:evaluate()
    seq2:evaluate()
...
function rnntest.RecurrentAttention()
    ...
    -- recurrent neural network
    local rnn = nn.Recurrent(opt.hiddenSize,
        glimpse,
...
function rnntest.LSTM_char_rnn()
    ...
    local function makeRnnLSTM(input_size, rnn_size, n)
        local seq = nn.Sequential():add(nn.OneHot(input_size))
...
function rnntest.Recurrent_checkgrad()
    ...
    local rnn = nn.Recurrent(hiddenSize, nn.LookupTable(nIndex, hiddenSize),
        nn.Linear(hiddenSize, hiddenSize))
    local seq = nn.Sequential():add(rnn):add(nn.Linear(hiddenSize, hiddenSize))
...
function rnntest.Recurrence()
    ...
    local seq2_ = nn.Sequential():add(nn.ParallelTable():add(inputLayer):add(feedbackLayer)):add(outputLayer)

    local seq2 = nn.Sequencer(nn.Sequential():add(rnn):add(outputLayer):add(nn.LogSoftMax()))

    local rm = nn.Sequential():add(nn.ParallelTable():add(inputLayer:clone()):add(feedbackLayer:clone())):add(nn.CAddTable()):add(nn.Sigmoid())

    local seq = nn.Sequencer(nn.Sequential():add(nn.Recurrence(rm, hiddenSize, 0)):add(outputLayer:clone()):add(nn.LogSoftMax()))
...
function rnntest.Recurrence_FastLSTM()
    ...
    local rm = nn.Sequential():add(nn.CSubTable()):add(lstm):add(nn.Linear(hiddenSize, hiddenSize)):add(nn.Sigmoid())
...
function rnntest.TrimZero()
    ...
    for ir, arch in pairs(rnns) do
        ...
        local model = nn.Sequential():add(nn.LookupTableMaskZero(vocabSize, word_embedding_size)):add(nn.SplitTable(2)):add(nn.Sequencer(rnn)):add(nn.SelectTable(-1)):add(nn.Linear(rnn_size, 10))
...
function rnntest.AbstractRecurrent_maskZero()
    ...
    local rm = nn.Sequential():add(nn.ParallelTable():add(nn.Linear(10, 10)):add(nn.Linear(10, 10))):add(nn.CAddTable()):add(nn.Sigmoid())
...
function rnntest.issue204()
    ...
    local rnn = nn.Sequential():add(nn.SplitTable(1, 2)):add(sequencer) --nn.FastLSTM(inputSize, hiddenSize))):add(nn.SelectTable(-1)):add(nn.Linear(hiddenSize, nClasses)):add(nn.LogSoftMax())
    local criterion = nn.ClassNLLCriterion()
    local params, gradParams = rnn:getParameters()
...
function rnntest.SeqLSTM_main()
    ...
    local function testmodule(seqlstm, batchfirst, seqlen, batchsize, lstm2, remember, eval, seqlstm2, maskzero)
        ...
        if batchfirst then
            ...
            seqlstm2 = seqlstm2 or nn.Sequential():add(nn.SplitTable(1, 2)):add(nn.Sequencer(lstm2)):add(nn.Sequencer(nn.View(batchsize, 1, outputsize))):add(nn.JoinTable(1, 2))
        else
            ...
            seqlstm2 = seqlstm2 or nn.Sequential():add(nn.SplitTable(1)):add(nn.Sequencer(lstm2)):add(nn.Sequencer(nn.View(1, batchsize, outputsize))):add(nn.JoinTable(1))
        end
...
function rnntest.FastLSTM_issue203()
    ...
    local seq = nn.Sequencer(nn.Sequential():add(nn.Linear(2, wordEmbDim)):add(nn.Copy(nil, nil, true)):add(nn.FastLSTM(wordEmbDim, lstmHidDim)):add(nn.Linear(lstmHidDim, nActions)):add(nn.LogSoftMax()))

    local seq2 = nn.Sequencer(nn.Sequential():add(nn.Linear(2, wordEmbDim)):add(nn.FastLSTM(wordEmbDim, lstmHidDim)):add(nn.Linear(lstmHidDim, nActions)):add(nn.LogSoftMax()))
...
function rnntest.NormStabilizer()
    ...
    local r = nn.Recurrent(hiddenSize, nn.Linear(inputSize, hiddenSize),
        nn.Linear(hiddenSize, hiddenSize), nn.Sigmoid(),
        rho)

    -- build simple recurrent neural network
    local rnn = nn.Sequential():add(r):add(nn.NormStabilizer(beta))
...
function rnntest.NCE_MaskZero()
    ...
    lm = nn.Sequential():add(nn.ParallelTable():add(lm):add(nn.Identity())):add(nn.ZipTable()) -- {{x1,x2,...}, {t1,t2,...}} -> {{x1,t1},{x2,t2},...}
...
function rnntest.SeqGRU_main()
    ...
    local function testmodule(seqGRU, batchfirst, seqlen, batchsize, gru2, remember, eval, seqGRU2, maskzero)
        ...
        if batchfirst then
            ...
            gradOutput = torch.randn(batchsize, seqlen, outputsize)
            seqGRU2 = seqGRU2 or nn.Sequential():add(nn.SplitTable(1, 2)):add(nn.Sequencer(gru2)):add(nn.Sequencer(nn.View(batchsize, 1, outputsize))):add(nn.JoinTable(1, 2))
        else
            input = torch.randn(seqlen, batchsize, inputsize)
            if maskzero then
                for i = 1, seqlen do
            ...
            gradOutput = torch.randn(seqlen, batchsize, outputsize)
            seqGRU2 = seqGRU2 or nn.Sequential():add(nn.SplitTable(1)):add(nn.Sequencer(gru2)):add(nn.Sequencer(nn.View(1, batchsize, outputsize))):add(nn.JoinTable(1))
        end
