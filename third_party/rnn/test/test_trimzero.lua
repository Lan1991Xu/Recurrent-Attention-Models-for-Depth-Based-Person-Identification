...
for ir, arch in pairs(rnns) do
    ...
    local model = nn.Sequential():add(nn.LookupTableMaskZero(vocabulary_size, word_embedding_size)):add(nn.SplitTable(2)):add(nn.Sequencer(rnn)):add(nn.SelectTable(sentence_length)):add(nn.Linear(rnn_size, 10))
